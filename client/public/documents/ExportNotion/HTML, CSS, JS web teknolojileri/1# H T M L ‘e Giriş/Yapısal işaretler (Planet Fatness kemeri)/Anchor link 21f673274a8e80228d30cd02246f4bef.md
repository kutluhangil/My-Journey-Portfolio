...
Siteniz tek sayfalık bir web sitesiyse, örneğin bir açılış sayfasıysa, ana navigasyonu, tıklandığında sayfayı yeniden yüklemeden sayfanın belirli bir bölümüne kaydıran bağlantılardır.Bu, sayfa navigasyonudur.

Bu tür bağlantıları oluşturmak için anchor links adı verilen bir yaklaşım kullanılır. Sayfadaki etiketin benzersiz bir tanımlayıcısı olan global `id` niteliğini kullanır. `id` niteliği herhangi bir etikete atanabilir, ancak sayfadaki değeri her biri için benzersiz olmalıdır.

Yani, bir "anchor link" oluşturmak için:

1. Kaydırmak istediğiniz etiketleri tanımlayın (örneğin, bölüm başlıkları)
2. Bu etiketlere `id` niteliğini ekleyin - benzersiz bir tanımlayıcı (aşağıdaki örneğe bakın)
3. Gezinti menüsündeki bağlantılar için `href` niteliğini # sembolüyle başlayan bir değere ve ardından öğe tanımlayıcısına ayarlayın.
...
Fitness kulüpleri sayfasının tüm biçimlendirmesini bu sayfa için oluşturduğunuz başlığa ekledik. Göreviniz, bir bağlantıya tıklandığında ana navigasyon menüsünün kullanıcıyı sayfanın bölümleri arasında hareket ettirmesini sağlamaktır. Bunu yapmak için, `h2` etiketlerine `id` niteliğini doğru değerlerle ekleyin ve gezinme bağlantıları için `href` niteliğinin değerini doldurun.

```
İlk bölüm için id niteliğinin değeri: clubs
İkinci bölüm için id niteliğinin değeri: benefits
Üçüncü bölüm için id niteliğinin değeri: programs

```

- Başlığın içinde `<nav>` etiketi bulunmalıdır.
- `<nav>` etiketi bir liste içermelidir.
- `Find us near you` başlığının `id` niteliğinin değeri `clubs` şeklinde olmalıdır.
- `Why choose Planet Fatness?` başlığının `id` özniteliğinin değeri `benefits` olmalıdır.
- `How to stop doubting and start exercising regularly` başlığının `id` niteliğinin değeri `programs`olmalıdır.
- `Our Clubs` bağlantısının `href` niteliğinin değeri `#clubs` şeklinde olmalıdır.
- `PF Benefits` referansının `href` niteliğinin değeri `#benefits` şeklinde olmalıdır.
- `Tips and Programs` bağlantısının `href` niteliğinin değeri `#programs` şeklinde olmalıdır.

<header>
<a href="/">
<img
src="[https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/b04pflogo.svg](https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/b04pflogo.svg)"
width="40"
height="40"
...
<li>
<a
href="[https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/Build-Muscles.pdf](https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/Build-Muscles.pdf)"
...
<li>
<a
href="[https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/Burn-Fat.pdf](https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/Burn-Fat.pdf)"
...
<li>
<a
href="[https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/Build-Strength&Mass.pdf](https://ac.goit.global/fullstack/html-css-v2/module-1/autocheck/Build-Strength&Mass.pdf)"
